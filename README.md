# Home Project and Vendor Management

Vendor account, contact and product tracking with project management and scheduling
...
```bash
mkvirtualenv domesticsentinel -p python3.11
...
Create a database named `domesticsentinel`.

```
createdb domesticsentinel
```
...
```bash
celery -A domesticsentinel worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A domesticsentinel worker -l INFO -B --pool=solo
```
